  ...
  <div class="diagnosis-list">
    <div class="diagnosis-search">
      <div class="search vux-1px-b">
        <div class="sea_l">
          <div class="sea_btn" v-if="keyWord.length > 0 && keyWord !== '请输入车牌号'">
            <img class="sea" src="../../assets/close_tips.png" @click="clearInput" alt>
          </div>
          <div class="sea_box">
            <img class="sea" src="../../assets/search2.png" alt>
            <!-- <input class="font-12" type="text" v-model="keyWord" placeholder="请输入车牌号"> -->
            <div
              class="font-12 input"
              :class="{'content': isContent}"
              @click="showKeyboard()"
              v-text="keyWord"
            ></div>
          </div>
  ...
  methods: {
    ...
    getList() {
      ...
      Api.getDiagnosisList({
        license_plate: self.keyWord !== "请输入车牌号" ? self.keyWord : ""
      }).then(res => {
        self.$loading.hide();
        if (res.code === 0) {
          res.data.map(item => {
