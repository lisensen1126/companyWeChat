  ...
  <div class="diagnosis-info">
    ...
    <div class="record-info">
      <div v-for="(step, findex) in current.category" :key="findex" class="diagnosis-step">
        <div class="diagnosis-step-title font-16" v-text="step.name"></div>
        <img v-if="step.icon" class="diagnosis-step-icon" :src="step.icon">
        <img v-if="!step.icon" class="diagnosis-step-icon" src="@/assets/default_car.png">
        <div v-for="(item, sindex) in step.category_item" :key="sindex" class="diagnosis-item">
          <div class="diagnosis-item-name font-14" v-text="item.name"></div>
          ...
          <div class="video" v-if="item.exception_video && item.exception_video.length > 0">
            <div class="diagnosis-point-name font-12" >视频</div>
            <div class="video-box">
              <video :src="item.exception_video[0]" controls ref="video" :poster="item.exception_video[0] + '?vframe/jpg/offset/1'"></video>
            </div>
          </div>
        </div>
      </div>
...
.diagnosis-step {
  position: relative;
  width: 100%;
  padding: 0 30/50rem 10/50rem 40/50rem;
